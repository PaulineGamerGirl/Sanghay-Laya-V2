export function TuckingSafety() {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mt-8 text-left animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="bg-red-50 rounded-3xl p-5 flex items-center gap-4 mb-8 border border-red-100 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 px-2">
        ...
        <div>
          <div className="flex items-center mb-6">
            <span className="font-body uppercase tracking-widest font-bold text-themeTextMain text-sm">
              THE DOS
            </span>
            <hr className="flex-grow border-themeAccent/20 ml-4 opacity-50" />
          </div>
          <ul className="space-y-4 mt-6">
            {dos.map((item, index) => (
              <li key={index} className="flex items-start gap-3">
                <span className="text-themeAccent shrink-0">✨</span>
                <span className="font-body italic text-themeTextMain/80 leading-relaxed">
                  {item}
                </span>
              </li>
            ))}
